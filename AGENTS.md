...
whistle (:8899, Node.js)  ──加载──►  whistle.pbmockx 插件（进程内）
                                              ├─ resRead/reqRead hook（gunzip→decode→patch→encode，pipe 单向）
...
pbmockx decode <id> [--req|--res] [--original]           # 显示 headers + body（PB 用 renderTree）
pbmockx rules add|list|del|save|reload ...               # patch 规则 CRUD
...
| type | 落点 | 关键字段 | 作用 |
|---|---|---|---|
| `patch` | resRead（pipe） | `path`, `value`, `protocol` | 在 message 对象上按 path set 字段，fromObject→encode |
...
- CLI 在 `whistle-plugin/bin/cli.js`（Node.js），通过 `w2 exec pbmockx` 或 npm link 后直接 `pbmockx` 调用。手写 argv 分发，`cmd_<verb>` 函数。**所有命令支持 `-h`/`--help`**。`decode <id>` 显示 headers + body，支持 `--req`/`--res`（只看请求或响应）/`--original`（pre-patch 原始数据），PB 输出用 `renderTree`（`field-tree.ts`）渲染成字段树。
...
- **Legacy addon**：`addon/pbmockx_addon.py` 类名仍为 `TapPbMock`（历史遗留），由 `scripts/start-mitmproxy.sh` 启动，仅作无 Node 环境的 fallback。新功能不再同步到 addon。
